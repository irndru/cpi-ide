...
plotTimeSeriesD :: G.DrawingArea -> LA.Vector Double -> LA.Matrix Double -> [Species]  -> IO Bool 
...
plotTimeSeriesFilteredD :: G.DrawingArea -> LA.Vector Double -> LA.Matrix Double -> [Species] -> [Species] -> IO Bool
plotTimeSeriesFilteredD  drawarea ts soln ss ss'
    = plotD 
      drawarea 
      (LA.toList ts)
      (filter (\(s,_)-> s `elem` (map specName ss')) 
...
plotD  :: G.DrawingArea -> [Double] -> [(String,[Double])] -> IO Bool
plotD da ts dims = updateCanvas (toRenderable (layout ts dims)) da 
...
-- a plot of two dimensions:
phasePlot2D :: G.DrawingArea 
	   -> LA.Vector Double 
           -> LA.Matrix Double 
           -> [Species] 
           -> (Species,Species) 
           -> IO Bool
phasePlot2D da ts soln ss ss'
    = plotPhaseD 
	da
      (filter (\(s,_)-> (s == (specName (fst ss'))) || s == (specName (snd ss')))
       (zip (map specName ss) (map LA.toList (LA.toColumns soln))))
...
plotPhaseToFile dims file = renderableToPDFFile (toRenderable (layout2phase dims)) 640 480 file

plotPhase dims = renderableToWindow (toRenderable (layout2phase dims)) 640 480

plotPhaseD da dims = updateCanvas (toRenderable (layout2phase dims)) da 
